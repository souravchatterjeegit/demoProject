...
describe('Book e2e test', () => {
  ...
  it('should load Books', async () => {
    ...
    expect(await bookComponentsPage.getTitle()).to.eq('blogApp.book.home.title');
  });

  it('should load create Book page', async () => {
    await bookComponentsPage.clickOnCreateButton();
    bookUpdatePage = new BookUpdatePage();
    expect(await bookUpdatePage.getPageTitle()).to.eq('blogApp.book.home.createOrEditLabel');
  ...
  it('should delete last Book', async () => {
    ...
    expect(await bookDeleteDialog.getDialogTitle()).to.eq('blogApp.book.delete.question');
